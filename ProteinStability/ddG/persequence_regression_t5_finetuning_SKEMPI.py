...
def train_model(parameter_dictionary):
    ...
    model = models.PerSequenceRegressionLora(parameter_dictionary, tokenizer).bfloat16().to(device)
    ...
    outmodel_name = 'ddG' + str(trial_num) + '.pt'
...
if __name__ == '__main__':
    ...
    tokenizer.add_tokens(['</break>'], special_tokens=True)

    train_df = shuffle(joblib.load('skempi_train.joblib'))
    val_df = shuffle(joblib.load('skempi_val.joblib'))
    test_df = shuffle(joblib.load('skempi_test.joblib'))
    
    training_set = models.PerSequenceRegressionLoraDataset(train_df, tokenizer, MAX_LEN, MUT_COL, WT_COL, LABEL_NAME, N_LABELS)
    validation_set = models.PerSequenceRegressionLoraDataset(val_df, tokenizer, MAX_LEN, MUT_COL, WT_COL, LABEL_NAME, N_LABELS)
    testing_set = models.PerSequenceRegressionLoraDataset(test_df, tokenizer, MAX_LEN, MUT_COL, WT_COL, LABEL_NAME, N_LABELS)
