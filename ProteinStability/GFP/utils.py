...
def loss_fn(predictions, truths, positive_scalings):

    threshold = 0.5
    loss_fct = torch.nn.BCEWithLogitsLoss(pos_weight=positive_scalings)
    ...
    for sample_num in range(length_batch):
        ...
        sample_pred_cpu = F.softmax(sample_pred.cpu().detach().float(), dim=0).numpy()
        sample_pred_cpu_binary = (sample_pred_cpu >= threshold).astype(int)
        
        if sample_pred_cpu_binary.argmax() == 0:
            pred_list.extend([0])
        else:
            pred_list.extend([1])

        sample_truth_cpu = sample_truth.cpu().detach().float().numpy()
        sample_truth_cpu_binary = (sample_truth_cpu >= threshold).astype(int)

        if sample_truth_cpu_binary.argmax() == 0:
            label_list.extend([0])
        else:
            label_list.extend([1])        
