...
def train_model(parameter_dictionary):
    ...
    model = models.PerSequenceClassificationLoraTPMT(parameter_dictionary, tokenizer).bfloat16().to(device)
    ...
    outmodel_name = 'TPMT_' + str(trial_num) + '.pt'
...
def objective(trial):
    ...
    parameters = {
            ...
            'in_shape' : TPMT_MAX_LEN,
            'out_shape' : N_LABELS,
            'epochs' : 50,
            'batch_size' : trial.suggest_int('batch_size', 2, 14, step=2),
...
if __name__ == '__main__':
    ...
    train_df = shuffle(joblib.load('../../Data/TPMT_train.joblib'))
    val_df = shuffle(joblib.load('../../Data/TPMT_val.joblib'))
    test_df = shuffle(joblib.load('../../Data/TPMT_test.joblib'))
    
    train_df.index = train_df.index.astype(str)
    val_df.index = val_df.index.astype(str)
    test_df.index = test_df.index.astype(str)

    training_set = models.PerSequenceClassificationLoraDataset(train_df, tokenizer, TPMT_MAX_LEN, SEQUENCE_COL, LABEL_NAME, N_LABELS)
    validation_set = models.PerSequenceClassificationLoraDataset(val_df, tokenizer, TPMT_MAX_LEN, SEQUENCE_COL, LABEL_NAME, N_LABELS)
    testing_set = models.PerSequenceClassificationLoraDataset(test_df, tokenizer, TPMT_MAX_LEN, SEQUENCE_COL, LABEL_NAME, N_LABELS)
