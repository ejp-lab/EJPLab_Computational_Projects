...
def train_model(parameter_dictionary):
    ...
    model = models.PerSequenceClassificationLoraRecA(parameter_dictionary, tokenizer).bfloat16().to(device)
    ...
    outmodel_name = 'RecA_' + str(trial_num) + '.pt'
...
if __name__ == '__main__':
    ...
    train_df = shuffle(joblib.load(cwd + '../../Data/train_RecA.joblib'))
    val_df = shuffle(joblib.load(cwd + '../../Data/val_RecA.joblib'))
    test_df = shuffle(joblib.load(cwd + '../../Data/test_RecA.joblib'))
    
    train_df.index = train_df.index.astype(str)
    val_df.index = val_df.index.astype(str)
    test_df.index = test_df.index.astype(str)

    training_set = models.PerSequenceClassificationLoraDataset(train_df, tokenizer, RECA_MAX_LEN, SEQUENCE_COL, LABEL_NAME, N_LABELS)
    validation_set = models.PerSequenceClassificationLoraDataset(val_df, tokenizer, RECA_MAX_LEN, SEQUENCE_COL, LABEL_NAME, N_LABELS)
    testing_set = models.PerSequenceClassificationLoraDataset(test_df, tokenizer, RECA_MAX_LEN, SEQUENCE_COL, LABEL_NAME, N_LABELS)
