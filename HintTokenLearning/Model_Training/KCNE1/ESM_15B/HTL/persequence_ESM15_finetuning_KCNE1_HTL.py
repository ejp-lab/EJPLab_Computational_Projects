...
def eval_model(parameter_dictionary, model, device, trial_num, epoch, set_flag):
    ...
    with torch.no_grad():
        for tmp_data_idx, tmp_data in enumerate(tmp_loader):
            ...
            tmp_loss, tmp_pred, tmp_truth = utils.loss_fn_multi_class(tmp_outputs, tmp_labels, positive_weightings.to(device))
...
def train_model(parameter_dictionary):
    ...
    model = models.PerSequenceClassificationLoraKCN1(parameter_dictionary, tokenizer).bfloat16().to(device)
    ...
    outmodel_name = 'KCN1_' + str(trial_num) + '.pt'
    ...
    for epoch in range(parameter_dictionary['epochs']):
        ...
        for train_data_idx, train_data in enumerate(training_loader):
            ...
            train_loss, train_pred, train_truth = utils.loss_fn_multi_class(train_outputs, train_labels, positive_weightings.to(device))
...
if __name__ == '__main__':
    ...
    NAME_COL = 'accession'
    SEQUENCE_COL = 'Mutated_Sequence_HTL'
    LABEL_NAME = 'label'
    MASK_NAME = 'mask'
    KCN1_MAX_LEN = 150
    N_LABELS = 3

    tokenizer = AutoTokenizer.from_pretrained("facebook/esm2_t48_15B_UR50D", do_lower_case=False)
    tokenizer.add_tokens(['[MUT_start]', '[MUT_end]'], special_tokens=True)
    
    train_df = shuffle(joblib.load(cwd + '../../../Data/trafficking_score/KCNE1_train.joblib'))
    val_df = shuffle(joblib.load(cwd + '../../../Data/trafficking_score/KCNE1_val.joblib'))
    test_df = shuffle(joblib.load(cwd + '../../../Data/trafficking_score/KCNE1_test.joblib'))

    train_df.index = train_df.index.astype(str)
    val_df.index = val_df.index.astype(str)
    test_df.index = test_df.index.astype(str)

    training_set = models.PerSequenceClassificationLoraDataset(train_df, tokenizer, KCN1_MAX_LEN, SEQUENCE_COL, LABEL_NAME, N_LABELS)
    validation_set = models.PerSequenceClassificationLoraDataset(val_df, tokenizer, KCN1_MAX_LEN, SEQUENCE_COL, LABEL_NAME, N_LABELS)
    testing_set = models.PerSequenceClassificationLoraDataset(test_df, tokenizer, KCN1_MAX_LEN, SEQUENCE_COL, LABEL_NAME, N_LABELS)
